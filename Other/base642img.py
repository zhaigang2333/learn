...
if __name__ == '__main__':
      ...
      pf=pd.read_excel('./tb_brand_trademark.xls')
      for r in pf[['ID','picture']].iterrows():
        try:
            id=r[1]['ID']
            picture=r[1]['picture']
            #print('base64:',picture)
            b=bytes(picture, encoding='utf-8')
            img = ImageObj(b)
            print('./images/'+str(id)+'.png')
            img.save('./images/'+str(id)+'.png')
        except Exception:
            print(Exception)
            continue
    # image = r'./test.png'
    # image = base64.b64encode(open(image, 'rb').read())
    # string=str(image,'utf-8')
    # print(type(string))
